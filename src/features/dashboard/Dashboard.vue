...
import { enrichmentCoverage, financialSummary, monthlyFinancials, paybackSummary, productAggregates, productAveragePrices, vatAggregates, type ProductAggregate } from '../../domain/receipts/basketAnalytics'
...
type ProductSort = 'spend' | 'occurrences' | 'avgPrice' | 'name'
...
const visibleProducts=computed(()=>[...filteredProducts.value].sort((a,b)=>{if(productSort.value==='name') return a.name.localeCompare(b.name,props.locale==='de'?'de-DE':'en-GB');if(productSort.value==='occurrences') return b.occurrences-a.occurrences||b.spendCents-a.spendCents||a.name.localeCompare(b.name);if(productSort.value==='avgPrice'){const avgA=productAveragePrices(a)[0]?.averagePriceCents??0;const avgB=productAveragePrices(b)[0]?.averagePriceCents??0;return avgB-avgA||b.spendCents-a.spendCents||a.name.localeCompare(b.name)}return b.spendCents-a.spendCents||b.occurrences-a.occurrences||a.name.localeCompare(b.name)}).slice(0,visibleProductCount.value))
...
<section class="dashboard">
  ...
  <section v-else-if="activeView==='products'" id="dashboard-panel-products" role="tabpanel" aria-labelledby="dashboard-tab-products">
    ...
    <article class="product-card"><header class="product-heading"><div><h2>{{ t('productRanking') }}</h2><p>{{ t('productRankingCopy') }}</p></div><div class="product-controls"><label><span class="sr-only">{{ t('searchProducts') }}</span><input v-model="productSearch" type="search" :placeholder="t('searchProducts')"></label><label><span class="sr-only">{{ t('sortProducts') }}</span><select v-model="productSort"><option value="spend">{{ t('productSortSpend') }}</option><option value="occurrences">{{ t('productSortOccurrences') }}</option><option value="avgPrice">{{ t('productSortAvgPrice') }}</option><option value="name">{{ t('productSortName') }}</option></select></label></div></header><div class="product-table-wrap"><table class="product-table"><thead><tr><th>{{ t('product') }}</th><th>{{ t('quantity') }}</th><th>{{ t('avgPrice') }}</th><th>{{ t('purchases') }}</th><th>{{ t('spend') }}</th></tr></thead><tbody><tr v-for="product in visibleProducts" :key="product.name"><td><button type="button" @click="openProduct(product)">{{ product.name }} <span aria-hidden="true">›</span></button></td><td>{{ quantitySummary(product) }}</td><td>{{ averagePriceSummary(product) }}</td><td>{{ integer.format(product.occurrences) }}</td><td>{{ money(product.spendCents) }}</td></tr></tbody></table><p v-if="!visibleProducts.length" class="empty-products">{{ t('noProductsFound') }}</p></div><button v-if="visibleProducts.length<matchedProductCount" class="load-more" type="button" @click="visibleProductCount+=50">{{ t('showMoreProducts') }}</button></article>
  </section>
